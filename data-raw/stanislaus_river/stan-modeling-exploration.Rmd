...
at_dam_location <- readRDS("data-raw/stanislaus_river/stan_temp_at_dam.rds") |> 
...
mid_river_location <- readRDS("data-raw/stanislaus_river/stan_temp_at_midriver.rds") |> 
  mutate(month = month(date), year = year(date)) |> 
  filter(year >= 1980) |> 
  mutate(calsim = "2019 CalSim II BiOp - Orange Blossom (OBS)",
         temp = monthly_mean_temp_c) |> 
  select(watershed, month, year, calsim, temp) |> 
  glimpse()
...
temperature <- expand_grid(
  ...
  bind_rows(at_dam_location, mid_river_location)
